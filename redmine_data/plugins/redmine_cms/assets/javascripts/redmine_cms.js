function activateCodeMirror(element, mode) {
  if (typeof(mode)==='undefined') mode = 'htmlmixedliquid';
...
CodeMirror.defineMode("scssmixedliquid", function(config, parserConfig) {
...
CodeMirror.defineMode("sassmixedliquid", function(config, parserConfig) {
  return CodeMirror.overlayMode(CodeMirror.getMode(config, parserConfig.backdrop || "text/x-sass"), CodeMirror.getMode(config, parserConfig.overlay || "application/x-liquid-template"));
});

CodeMirror.defineMode("javascriptmixedliquid", function(config, parserConfig) {
...
CodeMirror.defineMode("textilemixedliquid", function(config, parserConfig) {
  return CodeMirror.overlayMode(CodeMirror.getMode(config, parserConfig.backdrop || "text/x-textile"), CodeMirror.getMode(config, parserConfig.overlay || "htmlmixedliquid"));
});
...
function remove_page_fields (link) {
  $(link).prev("input[type=hidden]").val("1");
  $(link).parents('.page-field').hide();
}

function add_page_fields (link, association, content) {
  var new_id = new Date().getTime();
  var regexp = new RegExp("new_" + association, "g");
  var regexp = new RegExp("new_" + association, "g");
  $('.page-fields tbody tr').last().after(content.replace(regexp, new_id));
  $('#page_fields_attributes_' + new_id + '_name').focus();
}

$('#text_filter_select').bind('change', function() {
  editor.setOption('mode', $('option:selected', this).data('mode'));
  return false;
});


function generatePageSlug(title, maxlength, separator='-') {
  var diacriticsMap = [
  ...
  for(var i=0; i<diacriticsMap.length; i++) {
    title = title.replace(diacriticsMap[i].letters, diacriticsMap[i].base);
  }
  title = title.replace(/[^a-z0-9_]+/gi, separator); // remaining non-alphanumeric => hyphen
  title = title.replace(/^[-_\d]*|[-_]*$/g, ''); // remove hyphens/underscores and numbers at beginning and hyphens/underscores at end
  title = title.toLowerCase(); // to lower
  title = title.substr(0, maxlength); // max characters
  return title;
}

function autoFillPageSlug() {
  var slug_locked = ($('#page_slug').val() != '');
  var name_locked = ($('#page_name').val() != '');
  var maxlength = 200;

  $('#page_title').keyup(function(){
    if(!slug_locked) {
      $('#page_slug').val(generatePageSlug($('#page_title').val(), maxlength));
    }
    if(!name_locked) {
      $('#page_name').val(generatePageSlug($('#page_title').val(), maxlength, '_'));
    }
  });

  $('#page_slug').keyup(function(){
    slug_locked = ($('#page_slug').val() != '' && $('#page_slug').val() != generatePageSlug($('#page_title').val(), maxlength));
  });

  $('#page_name').keyup(function(){
    slug_locked = ($('#page_name').val() != '' && $('#page_name').val() != generatePageSlug($('#page_title').val(), maxlength, '_'));
  });
}
